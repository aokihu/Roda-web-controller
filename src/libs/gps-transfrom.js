...
export function gcj2wgsExact(gcjLat, gcjLng) {
  ...
  let newLat = gcjLat; let
    newLng = gcjLng;
  let oldLat = newLat; let
    oldLng = newLng;
...
export function distance(latA, lngA, latB, lngB) {
  ...
  const x = Math.cos(arcLatA) * Math.cos(arcLatB) * Math.cos((lngA - lngB) * pi180);
